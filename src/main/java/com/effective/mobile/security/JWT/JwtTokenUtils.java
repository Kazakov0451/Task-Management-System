package com.effective.mobile.security.JWT;
...
public class JwtTokenUtils {
    ...
    private boolean isTokenExpired(String token) {
        return getClaimsFromToken(token, accessKey).getExpiration().before(new Date());
    }

    private Claims getClaimsFromToken(@NotNull String token, SecretKey key) {
        return Jwts.parser()
                .verifyWith(key)
                .build()
